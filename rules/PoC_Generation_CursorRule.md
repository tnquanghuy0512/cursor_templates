...
4.  **Clarity and Actionability:** The PoC must be easy to understand and follow. If generating steps, make them precise. If generating code, ensure it's logically sound (even if pseudo-code is necessary due to missing details). Do not use em-dashes (—) in the output; rewrite with commas, parentheses, colons, or separate sentences instead.
